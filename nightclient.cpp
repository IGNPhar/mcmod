...
static int   g_frames = 0, g_beats = 0, g_touch_logged = 0;
...
static int32_t hook_getEvent(AInputQueue *q, AInputEvent **out) {
    for (;;) {
        ...
        if (AInputEvent_getType(ev) == AINPUT_EVENT_TYPE_MOTION) {
            ...
            if (swallow) {
                if (g_touch_logged < 6) { g_touch_logged++; nclog("touch taken: action=%d at %.0f,%.0f", action, x, y); }
...
static void build_ui(float w, float h, float fps, bool in_settings, NcRect *n_rect, NcRect *win_rect) {
    ...
    if (in_settings && !g_menu_open) {
        ...
        static bool n_logged = false;
        if (!n_logged) { n_logged = true; nclog("N button drawn at %.0f,%.0f size %.0fx%.0f", p.x, p.y, s.x, s.y); }
    ...
    if (g_menu_open) {
        ...
        ImGui::TextColored(ImVec4(0.62f, 0.55f, 1.0f, 1.0f), "Night Client 0.1.1   -   Minecraft PE 1.1.5");
        ImGui::Separator();
...
static void nc_frame(EGLDisplay d, EGLSurface s) {
    ...
    if (g_frames % 900 == 0 && g_beats < 6) {
        g_beats++;
        nclog("heartbeat: frames=%d settings=%d menu=%d imgui=%d fps=%.0f", g_frames, g_settings_this != 0, (int)g_menu_open, (int)g_imgui_ready, fps);
    }
    if (need && !g_drawing_logged) { g_drawing_logged = true; nclog("drawing started (settings=%d fps_on=%d)", g_settings_this != 0, g_cfg.fps_on); }
    ...
    if (gl_err != GL_NO_ERROR && !g_gl_err_logged) { g_gl_err_logged = true; nclog("GL error 0x%x after drawing (prev fbo %d)", (unsigned)gl_err, (int)prev_fbo); }
...
static void nc_init(void) {
    ...
    if (f) { fputs("Night Client 0.1.1 loaded\n", f); fclose(f); }
